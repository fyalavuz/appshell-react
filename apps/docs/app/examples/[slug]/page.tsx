...
import { DevicePreview } from "@/components/docs/device-preview";
...
export default async function ExamplePage({ params }: ExamplePageProps) {
  ...
  const previewUrl = withBasePath(`/examples/preview/${slug}/`);
  ...
  return (
    <article>
      ...
      {/* Device preview */}
      <div className="mt-10">
        <DevicePreview src={previewUrl} title={example.title} />
      </div>

      {/* Meta + hints */}
      <div className="mt-10 grid gap-8 rounded-2xl border bg-card p-6 sm:grid-cols-2 md:p-8">
        <div>
